...
- **Future Roadmap :** 
    - **Component Testing :** Verify that components like `Header.tsx` and `Inventory.tsx` correctly update their display in response to GameState changes(Planned).
    - **Implement a new function :** implement **Battle system** (Planned).
...
- **今後の展望 :**
    - **コンポーネントテストの導入** `Header.tsx` や `Inventory.tsx` が、`GameState` の変化に応じて正しく再レンダリングされるかの検証（予定）
    - **新機能の実装 :** 「戦闘システム」（予定）
